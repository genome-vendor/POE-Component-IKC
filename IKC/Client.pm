...
sub create_ikc_client
{
    ...
    $parms{name}||="Client$$";
    $parms{subscribe}||=[];
    new POE::Session( __PACKAGE__, [qw(_start _stop error connected)], [\%parms]);
}
...
sub _start {
  ...
  $heap->{wheel} = new POE::Wheel::SocketFactory
    ( RemotePort     => $parms->{port},
      RemoteAddress  => $parms->{ip},
...
sub connected
{
    ...
    create_ikc_channel($handle, $heap->{name}, $heap->{on_connect}, 
                                $heap->{subscribe}, $heap->{remote_name});
    delete @{$heap}{qw(name on_connect subscribe remote_name)};
...
managed to subscribe to each specifier.  If it can't, it will die().
